...
type SearchSort = "score_desc" | "created_at_desc" | "created_at_asc" | "updated_at_desc" | "updated_at_asc" | "name_asc" | "name_desc" | "connections_count_desc" | "random";
...
type Movement = "insert_at" | "move_to_top" | "move_to_bottom" | "move_up" | "move_down";
...
type BlockInput = { value: string, title?: string, description?: string, original_source_url?: string, original_source_title?: string, alt_text?: string };
...
type RateLimitError = { error: { type: string, message: string, tier: UserTier, limit: number, limit_window?: string, retry_after: number, current_status?: { tier?: string, limits?: {}, upgrade_path?: { current?: string, recommended?: string, benefits?: (string)[], action?: string } }, suggestions: (string)[], headers_note?: string } };
...
export const BlockImageSchema = v.object({ "alt_text": v.optional(v.union([v.string(), v.null_()])), "blurhash": v.optional(v.union([v.string(), v.null_()])), "width": v.optional(v.union([v.pipe(v.number(), v.integer()), v.null_()])), "height": v.optional(v.union([v.pipe(v.number(), v.integer()), v.null_()])), "src": v.optional(v.pipe(v.string(), v.url())), "aspect_ratio": v.optional(v.union([v.number(), v.null_()])), "content_type": v.optional(v.string()), "filename": v.optional(v.string()), "file_size": v.optional(v.union([v.pipe(v.number(), v.integer()), v.null_()])), "updated_at": v.optional(v.pipe(v.string(), v.isoTimestamp())), "small": ImageVersionSchema, "medium": ImageVersionSchema, "large": ImageVersionSchema, "square": ImageVersionSchema });
type BlockImage = { alt_text?: string | null, blurhash?: string | null, width?: number | null, height?: number | null, src?: string, aspect_ratio?: number | null, content_type?: string, filename?: string, file_size?: number | null, updated_at?: string, small: ImageVersion, medium: ImageVersion, large: ImageVersion, square: ImageVersion };
...
export const ChannelSchema = v.object({ "id": v.pipe(v.number(), v.integer()), "type": v.literal("Channel"), "slug": v.string(), "title": v.string(), "description": v.optional(v.union([MarkdownContentSchema, v.null_()])), "state": ChannelStateSchema, "visibility": ChannelVisibilitySchema, "created_at": v.pipe(v.string(), v.isoTimestamp()), "updated_at": v.pipe(v.string(), v.isoTimestamp()), "owner": ChannelOwnerSchema, "counts": ChannelCountsSchema, "_links": LinksSchema, "connection": v.optional(v.union([EmbeddedConnectionSchema, v.null_()])), "can": v.optional(v.union([ChannelAbilitiesSchema, v.null_()])) });
type Channel = { id: number, type: "Channel", slug: string, title: string, description?: MarkdownContent | null, state: ChannelState, visibility: ChannelVisibility, created_at: string, updated_at: string, owner: ChannelOwner, counts: ChannelCounts, _links: Links, connection?: EmbeddedConnection | null, can?: ChannelAbilities | null };
export const BaseBlockPropertiesSchema = v.object({ "id": v.pipe(v.number(), v.integer()), "base_type": v.literal("Block"), "title": v.optional(v.union([v.string(), v.null_()])), "description": v.optional(v.union([MarkdownContentSchema, v.null_()])), "state": BlockStateSchema, "visibility": BlockVisibilitySchema, "comment_count": v.pipe(v.number(), v.integer()), "created_at": v.pipe(v.string(), v.isoTimestamp()), "updated_at": v.pipe(v.string(), v.isoTimestamp()), "user": EmbeddedUserSchema, "source": v.optional(v.union([BlockSourceSchema, v.null_()])), "_links": LinksSchema, "connection": v.optional(v.union([EmbeddedConnectionSchema, v.null_()])), "can": v.optional(v.union([BlockAbilitiesSchema, v.null_()])) });
type BaseBlockProperties = { id: number, base_type: "Block", title?: string | null, description?: MarkdownContent | null, state: BlockState, visibility: BlockVisibility, comment_count: number, created_at: string, updated_at: string, user: EmbeddedUser, source?: BlockSource | null, _links: Links, connection?: EmbeddedConnection | null, can?: BlockAbilities | null };
...
export const BulkBlockResponseSchema = v.object({ "data": v.object({ "successful": v.array(v.object({ "index": v.pipe(v.number(), v.integer()), "block": BlockSchema })), "failed": v.array(v.object({ "index": v.pipe(v.number(), v.integer()), "error": v.string() })) }), "meta": v.object({ "total": v.pipe(v.number(), v.integer()), "successful_count": v.pipe(v.number(), v.integer()), "failed_count": v.pipe(v.number(), v.integer()) }) });
type BulkBlockResponse = { data: { successful: ({ index: number, block: Block })[], failed: ({ index: number, error: string })[] }, meta: { total: number, successful_count: number, failed_count: number } };
export default {
	ErrorSchema,
	LinkSchema,
