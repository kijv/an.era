...
lines.push('}');
lines.push('');
lines.push('/** `tag|group` → OpenAPI path param names (apply on this path binds params). */');
...
lines.push('}');
lines.push('');
lines.push('/** Same idea as `hono/client` `createProxy`: accumulate `path`, dispatch on `apply`. */');
