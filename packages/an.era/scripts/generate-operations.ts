...
for (const tag of orderedTags) {
  ...
  for (const op of rootOps) {
    ...
    lines.push("          return { kind: 'root-endpoint' };");
  }
  for (const group of groups) {
    const gn = groupNameForParams(group.params);
    lines.push(`        case ${JSON.stringify(gn)}:`);
    lines.push(`          return { kind: 'param-group', params: ${JSON.stringify(group.params)} };`);
  }
  lines.push('        default:');
...
lines.push('}');
lines.push('');
lines.push('function dispatchRootOp(client: Client, tag: string, opId: string, args: unknown[]): unknown {');
lines.push('  switch (tag) {');
...
for (const tag of orderedTags) {
  ...
  if (rootOps.length === 0) {
    lines.push(
      `      throw new Error(\`Tag ${tag.key} has no root-level endpoints (got \${opId})\`);`,
    );
    continue;
  }
  ...
  lines.push('        default:');
  lines.push(
    `          throw new Error(\`Unknown root endpoint \${opId} on tag ${tag.key}\`);`,
  );
  lines.push('      }');
}
...
for (const tag of orderedTags) {
  ...
  if (groups.length === 0) {
    lines.push(
      `      throw new Error(\`Tag ${tag.key} has no param groups (got group \${group})\`);`,
    );
    continue;
  }
  ...
  for (const group of groups) {
    ...
    lines.push('            default:');
    lines.push(
      `              throw new Error(\`Unknown endpoint \${opId} in group ${gn} (tag ${tag.key})\`);`,
    );
    lines.push('          }');
  }
  lines.push('        default:');
  lines.push(
    `          throw new Error(\`Unknown param group for tag ${tag.key}: \${group}\`);`,
  );
  lines.push('      }');
}
...
lines.push('');
lines.push('/** Third step: bound path params are fixed; each property is an endpoint invocation. */');
lines.push('function createGroupProxy(');
lines.push('  client: Client,');
lines.push('  tag: string,');
lines.push('  group: string,');
lines.push('  bound: Record<string, unknown>,');
...
lines.push('}');
lines.push('');
lines.push('/** Second step under a tag: each property is either a root endpoint or a param-group factory. */');
...
lines.push("      if (access.kind === 'root-endpoint') {");
lines.push('        return (...args: unknown[]) => dispatchRootOp(client, tag, key, args);');
lines.push('      }');
...
lines.push('}');
lines.push('');
lines.push('/** First step: each property is an OpenAPI tag. */');
