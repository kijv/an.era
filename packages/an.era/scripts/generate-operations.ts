...
const operationsTypeLines: string[] = [
...
const opTypeLines: string[] = ['export type Operations = {'];

const operationsToPath: Map<
  string,
  {
    path: string;
    method: string;
...
for (const path in paths) {
  ...
  for (const method in pathObj) {
    ...
    if (methodObj.operationId)
      operationsToPath.set(methodObj.operationId, { path, method });
  }
}

for (const [operationId, { path, method }] of operationsToPath.entries()) {
  const parts = path
    .split('/')
    .map((s) =>
    ...
    .filter(Boolean);
  opTypeLines.push(
    `  ${tsInterfacePropertyKey(operationId)}: Client${parts.map((s) => `[${JSON.stringify(s)}]`).join('')};`,
  );
}
