...
lines.push("  | { kind: 'root-op' }");
lines.push("  | { kind: 'group'; params: string[] };");
lines.push('');
...
for (const tag of orderedTags) {
  ...
  for (const op of rootOps) {
    ...
    lines.push("          return { kind: 'root-op' };");
  }
  for (const group of groups) {
    const gn = groupNameForParams(group.params);
    lines.push(`        case ${JSON.stringify(gn)}:`);
    lines.push(`          return { kind: 'group', params: ${JSON.stringify(group.params)} };`);
  }
  lines.push('        default:');
...
lines.push("      if (access.kind === 'root-op') {");
