...
const outTypesPath = path.join(pkgRoot, 'src', 'builder.ts');
const outDispatchPath = path.join(pkgRoot, 'src', 'builder.dispatch.ts');
...
for (const tag of orderedTags) {
  ...
  dispatchLines.push(`    case ${JSON.stringify(tag.key)}:`);
  dispatchLines.push('      switch (key) {');
  for (const op of rootOps) {
    dispatchLines.push(`        case ${JSON.stringify(op.operationId)}:`);
    dispatchLines.push("          return { kind: 'root-op' };");
  }
  for (const group of groups) {
    const gn = groupNameForParams(group.params);
    dispatchLines.push(`        case ${JSON.stringify(gn)}:`);
    dispatchLines.push(`          return { kind: 'group', params: ${JSON.stringify(group.params)} };`);
  }
  dispatchLines.push('        default:');
  dispatchLines.push('          return undefined;');
  dispatchLines.push('      }');
}
...
for (const tag of orderedTags) {
  ...
  if (rootOps.length === 0) {
    dispatchLines.push(
      `      throw new Error(\`Tag ${tag.key} has no root-level operations (got \${opId})\`);`,
    );
    continue;
  }
  dispatchLines.push('      switch (opId) {');
  for (const op of rootOps) {
    dispatchLines.push(`        case ${JSON.stringify(op.operationId)}:`);
    dispatchLines.push(
      `          return invokeEndpoint(client${op.clientPath}['$${op.method}'], {}, args);`,
    );
  }
  dispatchLines.push('        default:');
  dispatchLines.push(
    `          throw new Error(\`Unknown operation \${opId} on tag ${tag.key}\`);`,
  );
  dispatchLines.push('      }');
}

dispatchLines.push('    default:');
dispatchLines.push('      throw new Error(`Unknown tag ${tag}`);');
dispatchLines.push('  }');
dispatchLines.push('}');
dispatchLines.push('');
dispatchLines.push(
...
dispatchLines.push('  client: Client,');
dispatchLines.push('  tag: string,');
dispatchLines.push('  group: string,');
dispatchLines.push('  opId: string,');
dispatchLines.push('  bound: Record<string, unknown>,');
dispatchLines.push('  args: unknown[],');
dispatchLines.push('): unknown {');
dispatchLines.push('  switch (tag) {');
...
for (const tag of orderedTags) {
  ...
  if (groups.length === 0) {
    dispatchLines.push(
      `      throw new Error(\`Tag ${tag.key} has no grouped operations (got group \${group})\`);`,
    );
    continue;
  }
  dispatchLines.push('      switch (group) {');
  for (const group of groups) {
    const gn = groupNameForParams(group.params);
    dispatchLines.push(`        case ${JSON.stringify(gn)}:`);
    dispatchLines.push('          switch (opId) {');
  ...
  dispatchLines.push('        default:');
  dispatchLines.push(
    `          throw new Error(\`Unknown group for tag ${tag.key}: \${group}\`);`,
  );
  dispatchLines.push('      }');
}
...
await fs.writeFile(outTypesPath, lines.join('\n'), 'utf-8');
await fs.writeFile(outDispatchPath, dispatchLines.join('\n'), 'utf-8');
console.log(`Wrote ${path.relative(repoRoot, outTypesPath)}`);
console.log(`Wrote ${path.relative(repoRoot, outDispatchPath)}`);
