...
const orderedTags = [...tags.values()].sort((a, b) =>
...
for (const tag of orderedTags) {
  const groups = new Map<
    string,
    { params: string[]; ops: GeneratedOperation[] }
  >();
  ...
  lines.push(`  ${tag.key}: {`);
  for (const op of rootOps.sort((a, b) =>
  ...
  for (const group of [...groups.values()].sort((a, b) =>
    ...
    if (group.params.length === 1) {
      ...
      lines.push(
        `    ${groupKey}(value: ParamValue<Client${rep.clientTypePath}['$${rep.method}'], ${JSON.stringify(k)}>): {`,
      );
    } else {
    ...
    for (const op of group.ops.sort((a, b) =>
      a.operationId.localeCompare(b.operationId),
    )) {
      lines.push(
