...
async function resolveSource(input) {
  ...
  if (source.startsWith('http://') || source.startsWith('https://')) {
    ...
    if (!res.ok) throw new Error(`Failed to fetch OpenAPI JSON: ${res.status} ${res.statusText}`);
...
function schemaToTypeExpr(schemaObject, options = {}) {
  const node = transformSchemaObject(
    schemaObject,
    {
      ...options,
      ctx,
...
function requestBodyToInputUnionExpr(requestBody) {
  ...
  if (!resolved || !resolved.content || typeof resolved.content !== 'object') return `{}`;
  ...
  for (const [contentType, mediaType] of Object.entries(resolved.content)) {
    ...
    const target = contentType === 'application/x-www-form-urlencoded' ? 'form' : 'json';
...
function buildInputExpr({ inputBodyExpr, pathParams }) {
  const parts = [];
  if (inputBodyExpr && inputBodyExpr !== '{}') parts.push(wrapIfUnion(inputBodyExpr));
...
function operationToEndpointUnionExpr({ operation, pathItem }) {
  ...
  return endpointMembers.length === 1 ? endpointMembers[0] : endpointMembers.join(' | ');
}
...
for (const [openApiPath, pathItem] of Object.entries(schema.paths ?? {})) {
  ...
  for (const methodName of methodOrder) {
    ...
    if (operation && typeof operation === 'object' && 'deprecated' in operation && operation.deprecated === true) {
      continue;
    }
...
const output = [
  '/**',
  ' * This file was auto-generated by scripts/generate-hono-app-from-openapi.mjs.',
  ' * DO NOT EDIT BY HAND.',
  ' */',
  '',
  ...
  '',
  // `components` is imported only so generated type expressions produced by openapi-typescript's oapiRef work.
